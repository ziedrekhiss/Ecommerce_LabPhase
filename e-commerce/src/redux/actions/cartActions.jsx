...
import {GET_CART_ITEMS,  GET_CART_ITEMS_SUCCESS, ADD_TO_CART, CLEAR_CART, SET_CURRENT_PAGE } from '../actionsTypes'
...
export const addItemToCart = (id)=> async (dispatch)=>{
    try {
        ...
        config);
        console.log('add to cart', response.data.items)
...
export const fetchItems = (page, itemPerPage) =>  async (dispatch) => {
      try {
        ...
        const response = await axios.get(`http://localhost:4000/api/cart/getItem?page=${page}&itemPerPage=${itemPerPage}`,config);
        const items = response.data && Array.isArray(response.data.items) ? response.data.items : [];
        console.log('itemmmmms',items);
        if (items.length >0){
    ...
    export const deleteItem=(productId, page, itemPerPage)=> async (dispatch)=>{
        try {
            const token = localStorage.getItem("token");
            const config={
            headers :{
